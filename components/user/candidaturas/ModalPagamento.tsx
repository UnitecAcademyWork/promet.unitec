...
import { X, CreditCard, Smartphone, Building, Upload, Check } from "lucide-react";
...
type ModalPagamentoProps = {
  ...
  titulo: string;
  valor: number;
  onConfirm: (dados: { metodo: string; numero?: string; comprovativo?: File }) => void;
};

const ModalPagamento: React.FC<ModalPagamentoProps> = ({ isOpen, onClose, titulo, valor, onConfirm }) => {
  const [metodo, setMetodo] = useState<"mpesa" | "transferencia">("mpesa"); // somente valores válidos
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 w-full max-w-md shadow-xl relative transform transition-all duration-300">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Finalizar Pagamento</h2>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-4 rounded-xl mb-6">
          ...
          <p className="text-2xl font-bold text-gray-800 dark:text-white">{valor} MT</p>
        </div>

        {/* Métodos de Pagamento */}
        <div className="mb-6">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Método de Pagamento</h3>
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              <Smartphone className={`w-5 h-5 ${metodo === "mpesa" ? "text-purple-500" : "text-gray-500"}`} />
            ...
            >
              <Building className={`w-5 h-5 ${metodo === "transferencia" ? "text-green-500" : "text-gray-500"}`} />
            ...
            <div>
              ...
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl cursor-pointer hover:border-gray-400 dark:hover:border-gray-600 transition-colors">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">Clique para enviar ou arraste o arquivo</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">PNG, JPG, PDF (Max. 5MB)</p>
                </div>
                <input
                  type="file"
                  className="hidden"
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
          ...
          <button
            ...
            className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md hover:shadow-lg font-medium"
          >
            Confirmar Pagamento
          </button>
        </div>
