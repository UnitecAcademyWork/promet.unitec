...
import { Eye, EyeOff, User, Mail, Lock, UserPlus, CheckCircle, Sparkles, KeyRound, User2, UserCircle } from "lucide-react";
...
// Componente de SVG animado para o fundo - versão mais leve
...
export default function RegisterForm() {
  ...
  // Efeito para preencher automaticamente o username se não foi editado manualmente
  useEffect(() => {
    if (form.nome && form.apelido && !usernameEdited) {
      const username = `${form.nome}${form.apelido}`.toLowerCase().replace(/\s+/g, '');
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Marcar que o username foi editado manualmente se o campo for alterado
    if (name === 'username') {
  ...
  return (
    <>
      ...
      <div className="h-[] flex items-center justify-center py-8 p-4 relative dark:bg-gray-700/50 bg-slate-50 overflow-hidden">
        {/* SVG Animados no Fundo - mais sutis */}
        <FloatingShapes />
        ...
        <motion.div
          ...
          className="relative z-10 flex w-sreen max-w-4xl bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:bg-gray-900/90 dark:border-gray-700/50 overflow-hidden"
        >
          ...
          <div className="hidden md:flex flex-1 relative">
            ...
            {/* <div className="absolute inset-0 flex flex-col justify-end p-6 text-white">
              <motion.div
                ...
                className="max-w-xs"
              >
                <h2 className="text-xl font-bold mb-3">Junte-se à Nossa Comunidade</h2>
          ...
          <div className="flex-1 p-5 md:p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {usernameEdited 
                      ...
                      : "Preencha nome e apelido para gerar automaticamente"}
                  </p>
                </div>
                  ...
                  {form.password && (
                    <div className="mt-2 space-y-1">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700">
                        <div
                          className={`h-1.5 rounded-full transition-all duration-500 ${passwordStrength.color} ${passwordStrength.width}`}
                        />
                      </div>
                    </div>
                  )}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className={`w-full pl-9 pr-10 py-2 text-sm border rounded-lg focus:ring-1 focus:ring-brand-main/30 focus:border-brand-main/20 transition-all dark:bg-gray-800 dark:text-white ${
              ...
              {/* Checkbox de Termos */}
              <div className="flex items-start space-x-2 p-2 bg-gray-50 rounded-lg dark:bg-gray-800/50">
                <input
                  type="checkbox"
                  id="terms"
                  checked={acceptedTerms}
                  onChange={(e) => setAcceptedTerms(e.target.checked)}
                  className="mt-0.5 w-4 h-4 text-brand-main bg-white border-gray-300 rounded focus:ring-brand-main focus:ring-1 dark:bg-gray-700 dark:border-gray-600"
                  required
                />
                <label htmlFor="terms" className="text-xs text-gray-600 dark:text-gray-300 leading-tight">
                  Concordo com os{" "}
                  <a href="#" className="text-brand-main hover:text-brand-lime font-medium underline">
                    Termos
                  </a>{" "}
                  e{" "}
                  <a href="#" className="text-brand-main hover:text-brand-lime font-medium underline">
              ...
              <motion.button
                ...
                className="w-full bg-brand-main text-white py-2.5 rounded-lg font-medium text-sm shadow-md hover:shadow transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
